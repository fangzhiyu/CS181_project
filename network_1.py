...
class NN():
    ...
    def __init__(self,train_feature,train_label,validate_feature,validate_label):
        ...
        self.LAYER1_NODE = 100       #隐藏层有50个神经元
        self.LAYER2_NODE = 100
        self.LAYER3_NODE = 100
        self.BATCH_SIZE = 32
    ...
    def inference(self,input_tensor):#双隐层神经网络的向前传播过程
        with tf.variable_scope('layer1',reuse=tf.AUTO_REUSE):
            weights = tf.get_variable("weights",[self.INPUT_NODE,self.LAYER1_NODE],initializer = tf.truncated_normal_initializer(stddev = 0.1))
            tf.add_to_collection('losses',tf.contrib.layers.l2_regularizer(self.REGULARIZATION_RATE)(weights))
        ...
        with tf.variable_scope('layer2',reuse=tf.AUTO_REUSE):
            weights = tf.get_variable("weights", [self.LAYER1_NODE, self.LAYER2_NODE],initializer=tf.truncated_normal_initializer(stddev=0.1))
            tf.add_to_collection('losses',tf.contrib.layers.l2_regularizer(self.REGULARIZATION_RATE)(weights))
            biases = tf.get_variable("biases", [self.LAYER2_NODE], initializer=tf.constant_initializer(0.0))
            layer2 = tf.nn.relu(tf.matmul(layer1, weights) + biases)
            
        with tf.variable_scope('layer3',reuse=tf.AUTO_REUSE):
            weights = tf.get_variable("weights", [self.LAYER2_NODE, self.LAYER3_NODE],initializer=tf.truncated_normal_initializer(stddev=0.1))
            tf.add_to_collection('losses',tf.contrib.layers.l2_regularizer(self.REGULARIZATION_RATE)(weights))
            biases = tf.get_variable("biases", [self.LAYER3_NODE], initializer=tf.constant_initializer(0.0))
            layer3 = tf.nn.relu(tf.matmul(layer2, weights) + biases)

        with tf.variable_scope('layer4',reuse=tf.AUTO_REUSE):
            weights = tf.get_variable("weights",[self.LAYER3_NODE,self.OUTPUT_NODE],initializer = tf.truncated_normal_initializer(stddev = 0.1))
            tf.add_to_collection('losses',tf.contrib.layers.l2_regularizer(self.REGULARIZATION_RATE)(weights))
